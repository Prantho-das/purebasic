        ...
        <div class="container-fluid">
            <div class="card mb-3">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-10">
                            <a href="{{url('/admin/all/job')}}"></a></i>
                            Questions Uploads
                        </div>
                        <div class="col-md-2">
                            <a href="{{url('admin/question_bank_questions/'.$model->id)}}" class="btn btn-primary"><i
                                    class="fas fa-book"></i> All Questions</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{url('/admin/add/question_bank_questions/'.$model->id)}}" method="post"
                          enctype="multipart/form-data">
                        @csrf
                        <input type="text" name="type" hidden value="{{ $model["type"] }}">
                        <input type="number" name="subject_id" hidden value="{{ $model["subject_id"] }}">
                        <input type="text" name="subject" hidden value="{{ $model["subject"] }}">
                        <input type="number" name="chapter_id" hidden value="{{ $model["chapter_id"] }}">
                        <input type="text" name="chapter" hidden value="{{ $model["chapter"] }}">
