                    ...
                    <table class="table table-striped" style="background: #fff;border: 1px solid #ccc;">
                        ...
                        @php
                            ...
                            if($user->position == 'Doctor'){
                                $lectureBatch = App\LectureBatch::where('membershipe_id',$user->batch_id)
                                ->orderBy('id','desc')->get();
                            }elseif($user->position == 'Overseas'){
                                $lectureBatch = App\LectureBatch::where('membershipe_id',$user->batch_id)
                                ->orderBy('id','desc')->get();
                            }else{
                                $lectureBatch = App\LectureBatch::where('membershipe_id',$user->batch_id)
                        ...
                        @foreach($lectureBatch as $key=>$batch)
                            @php
                                if($user->position == 'Doctor'){
                                    $data=App\LectureSheet::where('id',$batch->lecture_id)->where('status',1)->where('levell', $user->qualification)->first();
                                }else{
                                    $data=App\LectureSheet::where('id',$batch->lecture_id)->where('levell', $user->levell)->where('status',1)->first();
                                }
                            @endphp
                            @if($data)
                                <tr>
