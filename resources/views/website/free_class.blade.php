@extends('layouts.register')
@section('content')

    <div class="container">
    
    <div class="centerText">
        <h3>Enjoy our free lectures. You have to enroll in our ongoing batches to watch premium lecture videos. Whatsapp
            <a target="_blank" href="https://api.whatsapp.com/send?phone=%2B8801673652555" id="whatsapp-button" style="width: 55px;"><img style="width:1.3rem;" src="data:image/svg+xml;utf8;base64,"></a>
        us for further details.</h3>
    </div>
    
        @foreach($info as $key=>$data)

            <div class="row lectureContainer marginBelow" id={{$key+1}}>
                
                <div class="col-4">&nbsp</div>

                <div class="col-4 marginAbove round lecture">
                    

                    @if(!empty($data->video))
                    
                        <div class="videoButtonContainer">
                            <p style="margin: 0; text-align: center">{{$data->title}}</p>
                            <a href="{{route('free_video',['batch_id'=>$batch_id,'id'=>$data->id])}}" class="videoButton">&#x25B6</a>

                        </div>
                    @endif

                    <!--div>
                        @if(!empty($data->links))<a href="{{$data->links}}"  class="note1Button">Note 1</a> @endif
                        @if(!empty($data->pdf))<a href="{{$data->pdf}}" class="note2Button">Note 2</a> @endif
                        @if(!empty($data->v_link)) <a href="{{$data->v_link}}" class="pdfButton">PDF</a> @endif
                        @if(!empty($data->isExam)) <a href="{{url('/lecture/exam/'.$data->id)}}" class="quizButton" >Quiz</a>@endif
                    </div-->
                    
                </div>
                
                <div class="col-4">&nbsp</div>

            </div>
        @endforeach
    
    </div>
                

@endsection