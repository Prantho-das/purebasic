...
<div class="row container" style="margin-top:0.5rem;">
    ...
    <div class="col-6 submitForm">
        ...
        <a id="resend" href="{{'/student/otp/' . $id . '/login'}}" style="font-size: 1.3rem; background: #3b93ef; color: white; padding: 0.5rem; border-radius: 0.3rem;">Resend OTP</a>
        ...
        <div id="otpSection">
            ...
            <form method="post" action="{{url('student/otp/verify/'.$student->id.'/'.$is_login)}}">
              @csrf
                    ...
                    <h3><label>We have sent an OTP to {{$student->mobile}}; Please enter the OTP and click Next..</label></h3>
                    <h3><span>Time Remaining </span><label timer id='timer' class="timer"></label></h3>
                    
                    <input name="otp" type="text" placeholder="OTP" required value="{{$otpRequired == 0 ? $otp : ""}}">
                @if ($errors->has('otp'))
                  <span role="alert">
