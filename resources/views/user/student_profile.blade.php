                                    ...
                                    @foreach ($lectureAnalytics as $stat)
                                        ...
                                        <div class="an-lecture-card {{ $loop->index >= 10 ? 'an-lm-hidden' : '' }}">
                                            ...
                                            <div class="an-prog-track">
                                                <div class="an-prog-fill {{ $barClass }}"
                                                    style="width:{{ $tp }}%"></div>
                                            </div>
                                            <div class="an-lc-meta">
                                                <span><i class="fa-regular fa-clock"></i> Watched:
                                                    <b>{{ $wtLabel }}</b></span>
                                                <span><i class="fa-regular fa-hourglass-half"></i> Total duration:
                                                    <b>{{ $tdLabel }}</b></span>
                                                <span><i class="fa-regular fa-film"></i> Lectures:
                                                    <b>{{ $stat['watched'] }} / {{ $stat['total'] }}</b></span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                @if (count($lectureAnalytics) > 10)
                                    <div class="an-load-more-wrap">
                                        <button class="an-load-more-btn" data-grid="lect-prog-grid" data-shown="10">
                                            ...
                                            Load More <span class="lm-count">({{ count($lectureAnalytics) - 10 }}
                                                more)</span>
                                        </button>
                                    </div>
                                @endif
                                            ...
                                            <div class="an-subject-group">
                                                <div class="an-subject-label">
                                                    ...
                                                    <span style="color:#94a3b8;font-weight:400">
                                                        ({{ count($subjectData['chapters']) }} chapters)
                                                    </span>
                                                </div>
                                                ...
                                                <div class="an-lecture-grid" id="{{ $gridId }}">
                                                    @foreach ($subjectData['chapters'] as $ch)
                                                        ...
                                                        @endphp
                                                        <div
                                                            class="an-lecture-card {{ $loop->index >= 10 ? 'an-lm-hidden' : '' }}">
                                                            <div class="an-lc-header">
                                                                <span class="an-lc-title">{{ $ch['chapter'] }}</span>
                                                                <span
                                                                    class="an-badge {{ $pct >= 80 ? 'an-badge-green' : ($pct >= 40 ? 'an-badge-yellow' : 'an-badge-gray') }}">{{ $pct }}%</span>
                                                            </div>
                                                            <div class="an-prog-track">
                                                ...
                                                @if (count($subjectData['chapters']) > 10)
                                                    <div class="an-load-more-wrap">
                                                        <button class="an-load-more-btn" data-grid="{{ $gridId }}"
                                                            data-shown="10">
                                                            <i class="fa-regular fa-chevron-down"></i>
                                                            Load More <span
                                                                class="lm-count">({{ count($subjectData['chapters']) - 10 }}
                                                                more)</span>
                                                        </button>
                                                    </div>
                                                @endif
...
@section('js')
    ...
    <style>
        ...
        .an-load-more-btn:hover {
            background: #e2e8f0;
            color: #1e293b;
        }

        .an-load-more-btn .lm-count {
            color: #94a3b8;
            font-weight: 500;
        }
    </style>
        ...
        (function() {
            ...
            document.querySelectorAll('.an-load-more-btn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    var hidden = cards.filter(function(c) {
                        return c.classList.contains('an-lm-hidden');
                    });
                    var shown = parseInt(this.getAttribute('data-shown') || '10');

                    if (hidden.length > 0) {
                        hidden.slice(0, 10).forEach(function(c) {
                            c.classList.remove('an-lm-hidden');
                        });
                        var stillHidden = grid.querySelectorAll('.an-lecture-card.an-lm-hidden').length;
                        this.setAttribute('data-shown', (shown + Math.min(10, hidden.length))
                        .toString());
                        if (stillHidden === 0) {
                            this.innerHTML = '<i class="fa-regular fa-chevron-up"></i> Show Less';
                        } else {
                            this.innerHTML =
                                '<i class="fa-regular fa-chevron-down"></i> Load More <span class="lm-count">(' +
                                stillHidden + ' more)</span>';
                        }
                    } else {
                        cards.forEach(function(c, i) {
                            if (i >= 10) c.classList.add('an-lm-hidden');
                        });
